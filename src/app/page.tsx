...
export default function Home() {
  ...
  const [sortBy, setSortBy] = useState<SortableFilamentFields>(
    () =>
      (searchParams?.get("sortBy") as SortableFilamentFields) ?? "identifier",
  );
  ...
  useEffect(() => {
    if (data) {
      const filteredFilaments = filterData(
        data,
        materialFilter,
        colorFilter,
        brandFilter,
      ...
      const sortedFilaments = sortFilaments(
        filteredFilaments,
        sortBy,
        sortOrder,
      );
              ...
              {data &&
                generateKeyCountPairs(
                  ...
                  filterData(
                    ...
                    inStockFilter,
                    "inStock",
                  ).map((f) => (f.inStock ? "inStock" : "outOfStock")),
                )
        ...
        <div className="flex justify-between">
          ...
          <div className="flex gap-2">
            <Select
              value={sortBy}
              onValueChange={(value) => {
                setSortBy(value as keyof filament);
            ...
            <>
              ...
              <Card className="informationCard">
                ...
                <CardContent className="text-2xl font-bold flex flex-wrap gap-1">
                  {filaments &&
                    generateKeyCountPairs(filaments.map((f) => f.material))
                      .filter(
                        ([material]) =>
                          material !== undefined &&
                          material !== null &&
